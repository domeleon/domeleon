...
export class InspectorComponentTree extends Component {
  ...
  override onAttached() { this.initCollapse() }
  ...
  private primitive(label: string, raw: unknown, d: number, variant: Variant): VElement | null {
  ...
  private object(label: string, obj: Record<string,unknown>, d: number, v: Variant): VElement {
    const header = this.primitive(label, '[Object]', d, v)!
    const props  = Object.entries(obj)
      .map(([k,val]) => this.primitive(k, val, d+1, v))
      .filter(Boolean) as VElement[]
    return div(header, ...props)
  }

  private array(ownerId: string, key: string, arr: unknown[], d: number, variant: Variant): VElement[] {
    ...
    const header = div(
      { onClick: () => this.toggle(sid), class: arrayHeader, style: indent(d+1) },
    ...
    const items = arr.map((item,i) => {
      const depth = d+2
      if (item instanceof Component) return this.node(item, depth, `${key}[${i}]`)
      if (isPrimitive(item)) return div({ class: arrayItem, style: indent(depth) }, `${i}: ${item}`)
      if (item && typeof item === 'object') return this.object(`${key}[${i}]`, item as any, depth, variant)
  ...
  private privateGroup(comp: Component, ownerId: string, d: number): VElement[] {
    ...
    const header = div(
      { onClick: () => this.toggle(id), class: componentTreeStyles.privateHeader, style: indent(d+1) },
    ...
    const [primitives, arrays] = entries.reduce<[VElement[], VElement[]]>((p,[k,v]) => {
      if (Array.isArray(v) && v.length)      p[1].push(...this.array(ownerId, k, v, d+1, 'private'))
      else                                   { const n = this.primitive(k, v, d+2, 'private'); if (n) p[0].push(n) }
      return p
    }, [[],[]])
  ...
  private node(comp: Component, d: number, label: string): VElement {
    ...
    const header = div(
      { key: id, class: componentTreeStyles.header, style: indent(d),
        onMouseDown: e => { e.preventDefault(); this.toggle(id) } },
    ...
    /* groups */
    const privGroup = this.privateGroup(comp, id, d)
    const arrays    = Object.entries(comp)
      .filter(([k,v]) => Array.isArray(v) && v.length && !k.startsWith('_'))
      .flatMap(([k,v]) => this.array(id,k,v as any[],d,'public'))
    const children  = this.children(comp).map(ch => this.node(ch.component, d+1, ch.name))
